<?php
    ...
    if(isset($_SESSION['user']) && isset($_POST['idFriend'])){
        ...
        $queryTest = 'SELECT f.accepted FROM Friend f WHERE f.idFriend='.$_SESSION['user']->idUser.' AND f.idUser='.$idF;
        ...
        if($test==0) {
            $queryUpdate='UPDATE Friend SET accepted=1,new=1 WHERE idUser='.$idF.' AND idFriend='.$_SESSION['idUser']->idUser;
            ...
            $queryAddFriend ='INSERT INTO Friend VALUES ('.$_SESSION['user']->idUser.','.$idF.',0,0,0,NOW())';
            $result = mysql_query($queryAddFriend) or die('Query Add failed (friend_add.php): ' . mysql_error());
        }
    }
?>
