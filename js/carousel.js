...
function setUpSlideShow()
{
	...
	prevControl = document.getElementById(prevControlID);
    ...
    for (var i=0; i < slidesCollection.length; i++)
    {
       ...
       // slidesControllersCollection[i].id = slideControlPrefix+(i+1);
		...
		nextControl.onclick = function(){clickSlideN(this);};
		prevControl.onclick = function(){clickSlideP(this);};
...
function clickSlideN(control)
{
    //showSlide(Number(control.id.substr(control.id.lastIndexOf("-")+1)),true);
	showSlide(crtSlideIndex+1,true);
}
function clickSlideP(control)
{
    //showSlide(Number(control.id.substr(control.id.lastIndexOf("-")+1)),true);
	showSlide(crtSlideIndex-1,true);
}
